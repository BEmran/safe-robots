...
bool MPU::SetSampleRate(const uint16_t rate) {
  ...
  // the INTERNAL_SAMPLE_RATE should actually depend on MpuGyroDLPF value. most
  // common is 1k but could be higher
...
std::optional<std::array<int16_t, 3>> MPU::ReadAccelRaw() {
  ...
  const std::vector<int16_t> raw =
    i2c_.ReadWords(ACCEL_XOUT_H, 6, EndianByteOrder::BIG);
  if (raw.empty()) {
    return {};
  }
...
std::optional<std::array<int16_t, 3>> MPU::ReadGyroRaw() {
  ...
  // Read the six raw data registers into data array
  const std::vector<int16_t> raw =
    i2c_.ReadWords(GYRO_XOUT_H, 6, EndianByteOrder::BIG);
  if (raw.empty()) {
    return {};
  }
  return CopyVectorToArray<int16_t, 3>(raw);
