...
#define TB_PITCH_X                                                             \
  0  ///< Index of the dmp_TaitBryan[] array corresponding to the Pitch (X)
     ///< axis.
#define TB_ROLL_Y                                                              \
  1  ///< Index of the dmp_TaitBryan[] array corresponding to the Roll (Y) axis.
#define TB_YAW_Z                                                               \
  2  ///< Index of the dmp_TaitBryan[] array corresponding to the Yaw (Z) axis.
...
typedef struct rc_mpu_config_t {
  ...
  ///@{
  int gpio_interrupt_pin_chip;  ///< gpio pin, default 3 on Robotics Cape and BB
                                ///< Blue
  ...
  int dmp_fetch_accel_gyro;     ///< set to 1 to optionally raw accel/gyro when
                                ///< reading DMP quaternion, default: 0 (off)
  int dmp_auto_calibrate_gyro;  ///< set to 1 to let DMP auto calibrate the gyro
...
typedef struct rc_mpu_data_t {
  ...
  double fused_TaitBryan[3];  ///< fused Tait-Bryan angles (roll pitch yaw) in
                              ///< radians
 ...
 * magentometer can also be read with rc_mpu_read_mag() if using an MPU9150 or
