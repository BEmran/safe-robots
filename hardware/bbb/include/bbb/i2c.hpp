...
struct I2CState {
  ...
  uint8_t devAddr{0};
  int bus{-1};
  I2CState(const int bus_) : initialized{false}, fd{-1}, devAddr{0}, bus{bus_} {
  }
};
...
class I2CManager {
 public:
  I2CManager();
  std::shared_ptr<I2C> CreateI2C(const int bus, const uint8_t devAddr);
  void SetDeviceAddress(const int bus, const uint8_t devAddr);
  void Close(const int bus);
...
class I2C {
 public:
  ...
  uint16_t ReadWord(const uint8_t regAddr);
  std::vector<uint16_t> ReadWords(const uint8_t regAddr, const size_t count);
  ...
  bool WriteWord(const uint8_t reg_addr, const uint16_t data);
  bool WriteWords(const uint8_t reg_addr, const std::vector<uint16_t>& data);
