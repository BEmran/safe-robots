...
class MPUIMU
{
 ...
 protected:
  ...
  float _aRes;
  float _gRes;
  int32_t _accelsensitivity;
  int32_t _gyrosensitivity;
  float _accelBias[3];
  float _gyroBias[3];
  ...
  virtual void pushGyroBiases(uint8_t data[12])
  {
    (void)data;
  }

  virtual void readAccelOffsets(uint8_t data[12], int32_t accel_bias_reg[3])
  {
    (void)data;
    (void)accel_bias_reg;
  }
  ...
  virtual void readMPURegisters(uint8_t subAddress, uint8_t count,
                                uint8_t* dest) = 0;

  static float getRes(uint8_t scale, uint16_t vals[4]);
  static int32_t getSen(uint8_t ascale, uint16_t vals[4]);
};  // class MPU
