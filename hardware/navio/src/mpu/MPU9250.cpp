...
{
  readMPURegisters(XA_OFFSET_H, 2,
...
void MPU9250::accelWakeOnMotion()
{
  ...
  // kHz
  uint8_t c = readMPURegister(ACCEL_CONFIG2);  // get current ACCEL_CONFIG2
                                               // register value
  ...
  writeMPURegister(INT_PIN_CFG,
                   0x12);  // INT is 50 microsecond pulse and any read to clear
  writeMPURegister(INT_ENABLE, 0x41);  // Enable data ready (bit 0) and wake on
                                       // motion (bit 6)  interrupt
...
float MPU9250::readTemperature()
{
  ...
  return ((float)t) / 333.87f +
         21.0f;  // Gyro chip temperature in degrees Centigrade)
}

void MPU9250::initMPU6500(Ascale_t ascale, Gscale_t gscale,
                          uint8_t sampleRateDivisor, bool passthru)
{
  // wake up device
  // writeMPURegister(PWR_MGMT_1, 0x00); // Clear sleep mode bit (6), enable all
  // sensors
  writeMPURegister(PWR_MGMT_1,
                   0x80);  // Clear sleep mode bit (6), enable all sensors
  delay(100);              // Wait for all registers to reset

  // get stable time source
  writeMPURegister(PWR_MGMT_1, 0x01);  // Auto select clock source to be PLL
                                       // gyroscope reference if ready else
  delay(200);
  ...
  writeMPURegister(SMPLRT_DIV,
                   sampleRateDivisor);  // Use a 200 Hz rate; a rate consistent
                                        // with the filter update rate
  ...
  uint8_t c =
      readMPURegister(GYRO_CONFIG);  // get current GYRO_CONFIG register value
  ...
  if (passthru)
  {
    ...
    writeMPURegister(INT_PIN_CFG, 0x12);  // INT is 50 microsecond pulse and any
                                          // read to clear
  }
...
// Checks percent deviation from factory trim values, +/- 14 or less deviation
// is a pass
bool MPU9250::selfTest(void)
{
  ...
  writeMPURegister(GYRO_CONFIG, 1 << FS); 
  // Set accelerometer rate to 1 kHz and bandwidth to 92 Hz 
  writeMPURegister(ACCEL_CONFIG2, 0x02);
  // Set full scale range for the accelerometer to 2 g
  writeMPURegister(ACCEL_CONFIG, 1 << FS);  

  for (int ii = 0; ii < 200; ii++)
  {  // get average current values of gyro and acclerometer

    // Read the six raw data registers into data array
    readMPURegisters(ACCEL_XOUT_H, 6,
                     &rawData[0]); 
  ...
  writeMPURegister(ACCEL_CONFIG, 0xE0);
  // Enable self test on all three axes and set gyro range to +/- 250 degrees/s
  // Delay a while to let the device stabilize
  writeMPURegister(GYRO_CONFIG,  0xE0);  
  ...
  selfTest[0] =
      readMPURegister(SELF_TEST_X_ACCEL);  // X-axis accel self-test results
  selfTest[1] =
      readMPURegister(SELF_TEST_Y_ACCEL);  // Y-axis accel self-test results
  selfTest[2] =
      readMPURegister(SELF_TEST_Z_ACCEL);  // Z-axis accel self-test results
  selfTest[3] = readMPURegister(
      MPU6500::SELF_TEST_X_GYRO);  // X-axis gyro self-test results
  selfTest[4] = readMPURegister(
      MPU6500::SELF_TEST_Y_GYRO);  // Y-axis gyro self-test results
  selfTest[5] = readMPURegister(
      MPU6500::SELF_TEST_Z_GYRO);  // Z-axis gyro self-test results

  // Retrieve factory self-test value from self-test code reads
  factoryTrim[0] = (float)(2620 / 1 << FS) *
                   (pow(1.01, ((float)selfTest[0] - 1.0)));  // FT[Xa] factory
                                                             // trim calculation
  factoryTrim[1] = (float)(2620 / 1 << FS) *
                   (pow(1.01, ((float)selfTest[1] - 1.0)));  // FT[Ya] factory
                                                             // trim calculation
  factoryTrim[2] = (float)(2620 / 1 << FS) *
                   (pow(1.01, ((float)selfTest[2] - 1.0)));  // FT[Za] factory
                                                             // trim calculation
  factoryTrim[3] = (float)(2620 / 1 << FS) *
                   (pow(1.01, ((float)selfTest[3] - 1.0)));  // FT[Xg] factory
                                                             // trim calculation
  factoryTrim[4] = (float)(2620 / 1 << FS) *
                   (pow(1.01, ((float)selfTest[4] - 1.0)));  // FT[Yg] factory
                                                             // trim calculation
  factoryTrim[5] = (float)(2620 / 1 << FS) *
                   (pow(1.01, ((float)selfTest[5] - 1.0)));  // FT[Zg] factory
                                                             // trim calculation
  ...
  for (int i = 0; i < 3; i++)
  {
    tolerances[i] = 100.0f * ((float)(aSTAvg[i] - aAvg[i])) / factoryTrim[i] -
...
void MPU9250::gyroMagSleep()
{
  ...
  writeAK8963Register(AK8963_CNTL, temp & ~(0x0F));  // Clear bits 0 - 3 to
...
void MPU9250::gyroMagWake(Mmode_t mmode)
{
  uint8_t temp = 0;
  temp = readAK8963Register(AK8963_CNTL);
  writeAK8963Register(AK8963_CNTL,
...
void MPU9250::readMagData(int16_t* destination)
{
  ...
  readAK8963Registers(AK8963_XOUT_L, 7,
...
void MPU9250::initAK8963(Mscale_t mscale, Mmode_t Mmode, float* magCalibration)
{
  ...
  delay(10);
  readAK8963Registers(AK8963_ASAX, 3,
                      &rawData[0]);  // Read the x-, y-, and z-axis calibration
                                     // values
 // Return xyz-axis sensitivity adjustment values, etc.
